...
pinned: false
description: 在日常生活中，常常有通过校园网进行局域网通信的需求，但是本校校园网内设备间无法直接正常通信，本文对此进行分析并通过调整路由规则尝试绕过隔离机制。
tags: [网络, 编程, 校园网]
category: 实验
...
本文探索的绕过隔离方案为：删除子网默认直连路由而设置路由要求通讯经由网关中转，从而绕开对局域网直连的限制，对于单向连通的情况，可以使用 ssh 反向隧道实现双向连通。

理论上可能跳数变多，但是局域网通信速度仍然比其他方案高得多。
:::

1. 如果只希望看到客户端隔离的绕过办法，只需要阅读：[单向通信解决](#134-尝试解决电脑到手机) 和 [双向通信解决](#135-尝试解决手机到电脑)，事实上两个操作是对称的，只要阅读一个就有解决这类问题的办法了。
...
- **防火墙**：为了实验结果正确，需要放行对方的包，最简单的方法是完全关闭防火墙。
