...
1. 如果只希望看到 AP 隔离的绕过办法，只需要阅读：[单向通信解决](#134-尝试解决电脑到手机) 和 [双向通信解决](#136-尝试解决手机到电脑)，事实上两个操作是对称的，只要阅读一个就有解决这类问题的办法了。
2. 如果遇到双向无法通信但是单向可以连通的，可以阅读：[单向连通的情况下发送文件](#135-在单向连通的情况下发送包)。
...
- [1. 第一部分](#1-第一部分)
  ...
  - [1.3. 实验步骤](#13-实验步骤)
    ...
    - [1.3.5. 在单向连通的情况下发送包](#135-在单向连通的情况下发送包)
    - [1.3.6. 尝试解决：手机到电脑](#136-尝试解决手机到电脑)
    - [1.3.7. 测试](#137-测试)
- [2. 第二部分](#2-第二部分)
  - [2.1. 后续](#21-后续)
    - [2.1.1. 奇怪的非对称性](#211-奇怪的非对称性)
    - [2.1.2. 原因分析](#212-原因分析)
    - [2.1.3. 绕过这个问题实现双向通信](#213-绕过这个问题实现双向通信)
  - [2.2. 宿舍网段](#22-宿舍网段)
- [参考文献](#参考文献)
...
在电脑上执行 `ping -c 3 172.17.40.187`，发现无法连通：

![ping](3.webp)

在手机上执行 `ping -c 3 172.17.54.68`，同样无法连通：

![ping](4.webp)

:::note
这里本来想再在两边同时抓包进行测试的，但是当时忘记了，后续的实验环节会进行抓包测试。新增：抓包结果为双方都没抓到对方的包。
:::
...
在电脑上执行 `ping -c 3 172.17.0.1`，发现可以连通：

![ping](5.webp)

在手机上执行 `ping -c 3 172.17.0.1`，同样可以连通：
...
此时，在电脑上重新 `ping -c 3 172.17.40.187`，发现已经手机可以收到包：

![tcpdump](7-1.webp)

当然电脑这边仍然是超时（因为响应包无法回来）：
...
尽管单向通信已经足够解决发送文件的问题了，但是显然以上操作极度繁琐，即使写成脚本，也不如直接双向通信方便。
