...
1. $h(n + 1) = h(n)$
    ...
    根据 [性质 1](#341-性质-1不存在三项连续不变的-hn)，$h(n + 2) \neq h(n + 1)$，那么 $h(n + 2) = h(n + 1) + 1$。

    故 $h(h(n + 1)) + h(n + 2) = h(h(n)) + h(n + 1) + 1$。
    
    由 [归纳假设](#36-证明-ffn--n)，$h(h(n)) + h(n + 1) = n + 2$，那么 $h(h(n + 1)) + h(n + 2) = n + 3$ 成立。

2. $h(n + 1) = h(n) + 1$，$h(n + 2) = h(n + 1)$

   令 $h(n) + 1 = j$，那么 $h(j) = h(j - 1) \text{或 } h(j - 1) + 1$。

   1. $h(j) = h(j - 1)$

        由 [性质 1](#341-性质-1不存在三项连续不变的-hn)，$h(j + 1) \neq h(j)$，那么 $h(j + 1) = h(j) + 1$。

        $f(j + 1) = h(j) + j + 1 = h(j - 1) + j + 1 = h(h(n)) + h(n + 1) + 1$。

        由 [归纳假设](#36-证明-ffn--n)，$h(h(n)) + h(n + 1) = n + 2$，那么 $f(j + 1) = n + 3$。

        根据 [性质 2](#342-性质-2hn-的增长滞后性)，$h(n) + 2 \le n$，而根据 [归纳假设](#36-证明-ffn--n)，有 $m \le n, \quad f(f(m)) = m$。

        因此，$f(n + 3) = f(f(j + 1)) = j + 1 = h(n + 1) + 1 = h(n + 2) + 1$。
        
        但是根据 [引理 1](#33-引理-1原数列的相关性质)，$f(n + 3) = h(n + 2) \text{或 } h(n + 2) + n + 3$，显然矛盾。
...
3. $h(n + 1) = h(n) + 1$，$h(n + 2) = h(n + 1) + 1$
    ...
    2. $h(j) = h(j - 1) + 1$
        ...
        由 [归纳假设](#36-证明-ffn--n)，$h(h(n)) + h(n + 1) = n + 2$，那么 $f(j) = n + 2$。

        根据 [性质 2](#342-性质-2hn-的增长滞后性)，$j = h(n) + 1 \le n$，而根据 [归纳假设](#36-证明-ffn--n)，有 $m \le n, \quad f(f(m)) = m$。

        因此，$f(n + 2) = f(f(j)) = j = h(n) + 1 = h(n + 1)$。
        
        而根据 [引理 1](#33-引理-1原数列的相关性质)，$f(n + 2) = h(n + 1) \Leftrightarrow h(n + 2) = h(n + 1)$，这与分类讨论的假设矛盾。
...
1. $f(n + 1) = h(n)$
    ...
    根据 [引理 1](#33-引理-1原数列的相关性质)，$h(n + 1) = h(n)$，继而因为 [性质 1](#341-性质-1不存在三项连续不变的-hn)，$h(n - 1) \neq h(n)$。

    那么，由 [引理 1](#33-引理-1原数列的相关性质)，有：$h(n - 1) = h(n) - 1$

    令 $j = h(n) - 1$，于是有：$h(j + 1) + j = h(h(n)) + h(n) - 1 = h(h(n)) + h(n + 1) - 1$。

    由 [归纳假设](#36-证明-ffn--n)，$h(h(n)) + h(n + 1) = n + 2$，那么 $h(j + 1) + j = n + 1$。

    因为 [引理 1](#33-引理-1原数列的相关性质)，$h(j + 1)$ 要么为 $h(j)$，要么为 $h(j) + 1$。

    1. $h(j + 1) = h(j)$

        那么 $n + 1 = h(j + 1) + j = h(j) + j = h(h(n) - 1) + h(n) - 1 = h(h(n - 1)) + h(n) - 1$。

        根据 [归纳假设](#36-证明-ffn--n)，$h(h(n - 1)) + h(n) = n + 1$，那么 $n + 1 = n + 1 - 1 = n$，显然矛盾。

    2. $h(j + 1) = h(j) + 1$

        那么由 [引理 1](#33-引理-1原数列的相关性质)，$f(j + 1) = h(j) + j + 1 = h(j + 1) + j = n + 1$。

        因此，$n + 1 = f(j + 1) = f(h(n)) = f(f(n + 1))$ 成立。

2. $f(n + 1) = h(n) + n + 1$

    那么，根据 [引理 1](#33-引理-1原数列的相关性质)，$h(n + 1) = h(n) + 1$。

    令 $j = h(n) + n$，由 [引理 1](#33-引理-1原数列的相关性质)，$j + 1 = h(n) + n + 1 = f(n + 1)$。

    由 [引理 1](#33-引理-1原数列的相关性质)，$h(j + 1)$ 的取值必然在 $h(j)$ 和 $h(j) + 1$ 之间。

    1. $h(j + 1) = h(j) = n + 1$

        由 [引理 1](#33-引理-1原数列的相关性质)，有 $f(j + 1) = h(j) = h(j + 1)$。
    ...
    2. $h(j + 1) = h(j) + 1$
        ...
        由 [引理 1](#33-引理-1原数列的相关性质)，$f(j + 1) = h(j) + j + 1 > h(j)$。

        由 [性质 3](#343-性质-3hn-的下界保持性)，$\forall l \ge j + 1, \quad f(l) > h(j)$。

        由于 [性质 5](#345-性质-5fn-是对正整数集的全排列)，$f(n)$ 中会出现 $h(j)$，而 $l \ge j + 1$ 时，$f(l) > h(j)$，因此 $h(j) \in \{ f(1), f(2), \dots, f(j) \}$。
